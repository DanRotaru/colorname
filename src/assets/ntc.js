/*
+-----------------------------------------------------------------+
|     Created by Chirag Mehta - https://chir.ag/projects/ntc      |
|     Modified by DanRotaru - https://danrotaru.github.io/        |
|-----------------------------------------------------------------|
...
const ntc = {
  ...
  name: function (color) {
    ...
    if (!ntc.isValidColor(color))
      return false;

    color = ntc.normalizeColor(color);
    ...
    for (let i = 0; i < ntc.names.length; i++) {
      const distance = ntc.colorDistance(r, g, b, h, s, l, i);
    ...
    if (closestNameIndex >= 0) {
      return [("#" + ntc.names[closestNameIndex][0]).toUpperCase(), ntc.names[closestNameIndex][1], this.getCamelCaseName(ntc.names[closestNameIndex][1])];
    } else {
      return false;
    }
  },
  ...
  isValidColor: function (color) {
    return color.length >= 3 && color.length <= 7;
  },
  ...
  colorDistance: function (r, g, b, h, s, l, i) {
    const dr = r - ntc.names[i][2],
      dg = g - ntc.names[i][3],
      db = b - ntc.names[i][4],
      dh = h - ntc.names[i][5],
      ds = s - ntc.names[i][6],
    ...
    return dr * dr + dg * dg + db * db + dh * dh + ds * ds + dl * dl;
  },
  ...
  getCamelCaseName: function (name) {
    ...
    for (let i = 1; i < words.length; i++) {
      const word = words[i];
      camelCaseName += word.charAt(0).toUpperCase() + word.slice(1).toLowerCase();
    }
